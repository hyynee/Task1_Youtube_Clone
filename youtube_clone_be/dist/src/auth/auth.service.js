...
let AuthService = class AuthService {
    ...
    async register(createUserDto) {
        ...
        try {
            ...
            if (existingUser) {
                throw new common_1.HttpException({ statusCode: 400, message: 'Email đã tồn tại' }, common_1.HttpStatus.BAD_REQUEST);
            }
    ...
    async updateUser(id, body) {
        ...
        try {
            const user = await this.userModel.findByIdAndUpdate(id, { name, email, avatar }, { new: true });
            return user;
        }
        catch (error) {
